    ...
    <form action="{{ route('writers.update', $writer['id']) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <textarea name="bio" rows="3" placeholder="Short description about the writer..."
                class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 resize-none">{{ old('bio', $writer['bio'] ?? '') }}</textarea>
        </div>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-600 mb-2">Current Photo</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-gray-600 mb-1">{{ !empty($writer['photo']) ? 'Replace Photo' : 'Profile Photo' }}</label>
